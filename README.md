...
```bash
python main.py validate --date today
```

`status=BLOCKED`이면 활성 미체결 또는 재확인 필요 주문을 해결하기 전 실주문을 시작하지 마십시오. 운영 절차는 [Operations Runbook](docs/operations-runbook.md)을 참고합니다.
